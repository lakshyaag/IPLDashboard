...
teams <-
  balls %>% select(team1) %>% unique() %>% rename('team' = team1) %>% arrange(team)
...
get_batsman_runs <- function(batsman_name) {
  batsman_data <- balls %>%
    ...
    labs(
      x = 'Season',
      y = 'Runs scored',
      title = paste('Runs scored by', batsman_name)
    ) +
...
get_batsman_distribution_runs <- function(batsman_name) {
  batsman_data <- balls %>%
    ...
    mutate(batsman_runs = fct_rev(fct_relevel(
      batsman_runs,
    ...
    labs(
      x = "",
      y = "% of Runs",
      title = paste('Distribution of runs for', batsman_name),
      fill = ''
    ) +
    geom_hline(yintercept = 0, colour = "#000000") +
    theme_ipl() +
...
#### 1.5. Bowler performance
get_bowler_performance <- function(batsman_name) {
  favorite_bowlers <- balls %>%
    filter(batsman == batsman_name) %>%
  ...
  toughest_bowlers <- balls %>%
    filter(batsman == batsman_name) %>%
    filter(batsman == player_dismissed) %>%
    select(bowler) %>%
    group_by(bowler) %>%
    summarise(wickets = n()) %>%
    ungroup() %>%
  ...
  bowler_performance <- favorite_bowlers %>%
    left_join(toughest_bowlers, by = 'bowler')
...
get_runs_against_teams <- function(batsman_name) {
  batsman_data <- balls %>%
    ...
    labs(
      x = 'Team',
      y = 'Runs scored',
      title = paste('Runs scored against teams by', batsman_name)
    ) +
    geom_hline(yintercept = 0, colour = "#000000") +
    theme_ipl() +
    coord_flip()
...
get_runs_per_over <- function(batsman_name) {
  ...
  individual_overs <- batsman_data %>%
    ...
    ggplot(aes(
      x = over,
      y = batsman_runs,
      text = paste0("Over: ", over, "\nRuns: ", batsman_runs)
    )) +
    geom_col(fill = "#247da2") +
    labs(
      x = "Over",
      y = "Runs scored",
      title = paste("Runs per over of", batsman_name)
    ) +
...
get_bowler_runs <- function(bowler_name) {
  bowler_data <- balls %>%
    ...
    labs(
      x = 'Season',
      y = 'Runs conceded',
      title = paste('Runs concded by', bowler_name)
    ) +
    geom_hline(yintercept = 0, colour = "#000000") +
    theme_ipl()
...
get_economy_rate <- function(bowler_name) {
  bowler_data <- balls %>%
    ...
    labs(
      x = 'Season',
      y = 'Economy Rate',
      title = paste("Economy Rate of", bowler_name)
    ) +
    theme_ipl()
...
get_wickets_by_season <- function(bowler_name) {
  bowler_data <- balls %>%
    ...
    labs(
      x = 'Season',
      y = 'Number of wickets',
      title = paste('Distribution of wickets by', bowler_name),
      fill = 'Type of wicket'
    ) +
    geom_hline(yintercept = 0, colour = "#000000") +
    theme_ipl() +
...
get_wickets_by_over <- function(bowler_name) {
  bowler_data <- balls %>%
    ...
    labs(
      x = 'Over',
      y = 'Number of wickets',
      title = paste('Wickets per over by', bowler_name),
      fill = 'Type of wicket'
    ) +
    geom_hline(yintercept = 0, colour = "#000000") +
    theme_ipl() +
...
#### 2.5. Batsman performance
get_batsman_performance <- function(bowler_name) {
  wickets_by_batsman <- balls %>%
    filter(bowler == bowler_name) %>%
    select(batsman, dismissal_kind) %>%
    filter(!is.na(dismissal_kind), dismissal_kind != 'run out') %>%
    group_by(batsman) %>%
    summarise(wickets = n()) %>%
    arrange(desc(wickets))
  
  runs_by_batsman <- balls %>%
    filter(bowler == bowler_name) %>%
    select(batsman, batsman_runs) %>%
    group_by(batsman) %>%
    summarise(batsman_runs = sum(batsman_runs)) %>%
    arrange(batsman_runs)
  
  batsman_performance <- wickets_by_batsman %>%
    left_join(runs_by_batsman, by = 'batsman')
...
get_strike_rate_batsman_bowler <-
  function(batsman_name, bowler_name) {
    players_data <- balls %>%
...
get_performance_over_time_batsman_bowler <-
  function(batsman_name, bowler_name) {
    players_data <- balls %>%
      filter(batsman == batsman_name & bowler == bowler_name)
    
    if (nrow(players_data) != 0) {
      wickets_data <- players_data %>%
        select(season, dismissal_kind) %>%
        filter(!is.na(dismissal_kind), dismissal_kind != "run out") %>%
        group_by(season) %>%
        summarise(wickets = n()) %>%
      ...
      runs_data <- players_data %>%
        select(season, batsman_runs) %>%
        group_by(season) %>%
        summarise(batsman_runs = sum(batsman_runs)) %>%
        arrange(desc(batsman_runs))
      
      performance_over_time <- runs_data %>%
        left_join(wickets_data, by = c('season')) %>%
        arrange(season)
...
get_season_stats <- function(season_selected) {
  ...
  orange_cap <- balls %>%
    ...
    rename(
      'orange_cap' = batsman,
      'orange_cap_team' = batting_team,
      'orange_cap_runs' = runs
    )
  ...
  purple_cap <- balls %>%
    ...
    rename(
      'purple_cap' = bowler,
      'purple_cap_team' = bowling_team,
      'purple_cap_wickets' = wickets
    )
...
team_head_to_head <- function(team_1, team_2) {
  h2h_data <- balls %>%
    filter(team1 %in% c(team_1, team_2) &
             team2 %in% c(team_1, team_2)) %>%
    select(season:venue) %>%
    distinct()
}
...
get_head_to_head_history <- function(team_data, team_1, team_2) {
  h2h_data <- team_data %>%
    mutate(position = case_when(winner == team_1 ~ 1,
                                winner == team_2 ~ -1,
                                TRUE ~ 0)) %>%
    mutate(win_by = case_when(
      win_by_runs > 0 ~ win_by_runs,
      win_by_wickets > 0 ~ win_by_wickets,
      TRUE ~ 0
    )) %>%
    mutate(
      position_win = position * win_by,
      how_win = case_when(
        win_by_runs > 0 ~ paste0('Won by ', win_by_runs, ' runs'),
  ...
  h2h_plot <-
    ...
    geom_segment(data = h2h_data, aes(
      y = position_win,
      yend = 0,
      xend = date,
      color = winner
    ...
    geom_segment(
      x = 0,
      xend = (h2h_data %>% slice(n()) %>% select(date) %>% as.integer()),
      y = 0,
      yend = 0,
      size = 0.05,
      linetype = 'dotted'
...
get_toss_stats <- function(team_data, team_1, team_2) {
  toss_stats <- team_data %>%
    ...
    ggplot(aes(
      x = str_to_sentence(toss_decision),
      y = count,
      fill = winner,
      text = paste0(
        'Toss winner: ',
...
get_venue_wins <- function(team_data, team_1, team_2) {
  plot_venues <- team_data %>%
    ...
    labs(
      x = "Venue",
      y = "Number of wins",
      title = paste(team_1, 'vs.', team_2, 'by venue')
    ) +
    theme_ipl()
...
get_team_batting_performance <-
  function(team, start_year, end_year) {
...
sidebar <- dashboardSidebar(sidebarMenu(
  ...
  menuItem(
    'Explore by',
    tabName = 'overall_analysis',
    icon = icon('search'),
    startExpanded = TRUE,
    menuItem("Batsman", tabName = 'batsman', icon = icon('eraser')),
    #1
    menuItem(
      "Bowlers",
      tabName = 'bowler',
    ...
    #7
    menuItem("Top players", tabName = 'topplayers', icon = icon('medal')),
    #6
    menuItem(
      "Season statistics",
      tabName = 'season',
  ...
  menuItem(
    ...
    href = 'https://github.com/lakshyaag/ipldashboard',
    newtab = T,
    icon = icon('github')
  ),
  menuItem(
    ...
    tabItem('about',
            fluidRow(
              box(
                ...
                p(
                  "Embark on a data-driven journey and discover the untold stories behind the most memorable moments in cricket's most prestigious league!"
                )
              ),
    ...
    tabItem('batsman',
            ...
            fluidRow(
              tabBox(
                ...
                tabPanel(
                  "Favorite venues (by runs)",
                  dataTableOutput('fav_venues') %>% withSpinner()
                ),
                tabPanel(
                  "Bowler performance",
                  dataTableOutput('bowler_performance') %>% withSpinner()
                ),
                tabPanel(
                  "Runs against teams",
    ...
    tabItem('bowler',
            ...
            fluidRow(
              tabBox(
                ...
                tabPanel(
                  "Batsman performance",
                  dataTableOutput('batsman_performance') %>% withSpinner()
                )
              )
            )),
            ...
            fluidRow(
              tabBox(
                ...
                tabPanel(
                  ...
                  dataTableOutput('performance_over_time') %>% withSpinner()
                ),
              )
            )),
    tabItem('season',
...
server <- function(input, output, session) {
  ...
  output$fav_venues <-
    renderDataTable({
      get_favorite_venues(input$batsman_selected)
    },
    selection = 'none',
  ...
  output$bowler_performance <-
    renderDataTable({
      get_bowler_performance(input$batsman_selected)
    },
    selection = 'none',
    colnames = c('Bowler', "Runs scored", "Wickets"),
    rownames = F,
    options = list(dom = 'tp'),
    style = 'bootstrap')
  ...
  output$batsman_performance <-
    renderDataTable({
      get_batsman_performance(input$bowler_selected)
    },
    selection = 'none',
    colnames = c('Batsman', "Wickets taken", "Runs conceded"),
    rownames = F,
    options = list(dom = 'tp'),
    style = 'bootstrap')
  ...
  output$strike_rate_compare <- renderPlotly({
    validate(
      need(
        get_strike_rate_batsman_bowler(
          input$batsman_selected_compare,
          input$bowler_selected_compare
        ) != "",
        "Selected players did not play against each other"
      )
    )
    get_strike_rate_batsman_bowler(input$batsman_selected_compare,
                                   input$bowler_selected_compare)
  })
  ...
  output$dist_wickets_compare <- renderPlotly({
    ...
    get_wickets_batsman_bowler(input$batsman_selected_compare,
                               input$bowler_selected_compare)
  })
  
  
  output$performance_over_time <- renderDataTable({
    validate(
      need(
        get_wickets_batsman_bowler(
          input$batsman_selected_compare,
    ...
    get_performance_over_time_batsman_bowler(input$batsman_selected_compare,
                                             input$bowler_selected_compare)
  },
  selection = 'none',
  colnames = c("Season", "Runs conceded", "Wickets taken"), 
  rownames = F,
  options = list(dom = 'tp'),
  style = 'bootstrap')
  ...
  season_data <-
    reactive({
      get_season_stats(input$season_selected)
    })
  ...
  output$orangecap_box <- renderValueBox({
    valueBox(
      paste0(
        season_data()$orange_cap,
        ' (',
        season_data()$orange_cap_runs,
        ')'
      ),
      "Orange Cap",
      color = "orange",
  ...
  output$purplecap_box <- renderValueBox({
    valueBox(
      paste0(
        season_data()$purple_cap,
        ' (',
        season_data()$purple_cap_wickets,
        ')'
      ),
      "Purple Cap",
      color = "purple",
  ...
  output$most_dots_box <- renderValueBox({
    valueBox(paste0(season_data()$most_dots, ' (', season_data()$dots, ')'),
             "Most dots",
             color = "olive")
  })
  ...
  team_head_to_head_data <-
    reactive({
      team_head_to_head(input$team_1, input$team_2)
    })
  
  h2h_stats_data <-
    reactive({
      h2h_stats(team_head_to_head_data())
    })
  
  output$team1_wins <- renderValueBox({
    validate(need(
      h2h_stats_data() != "",
      "Selected teams did not play against each other"
    ))
    valueBox(
      paste0(
        h2h_stats_data() %>% filter(winner == input$team_1) %>% select(count)
      ),
  ...
  output$total_matches <- renderValueBox({
    ...
    valueBox(
      paste0(
        h2h_stats_data() %>% filter(winner == 'Total') %>% select(count)
      ),
  ...
  output$team2_wins <- renderValueBox({
    ...
    valueBox(
      paste0(
        h2h_stats_data() %>% filter(winner == input$team_2) %>% select(count)
      ),
  ...
  reactive_top_batsmen <-
    reactive({
      get_top_batsmen(input$top_player_year_input[1],
                      input$top_player_year_input[2])
    })
  reactive_top_bowler <-
    reactive({
      get_top_bowlers(input$top_player_year_input[1],
                      input$top_player_year_input[2])
    })
  
  output$top_batsmen <- renderDataTable({
    reactive_top_batsmen()
  },
  selection = 'none',
  colnames = c(
  ...
  output$top_bowlers <- renderDataTable({
    reactive_top_bowler()
  },
  selection = 'none',
  colnames = c(
  ...
  reactive_team_batting_performance <-
    reactive({
      get_team_batting_performance(
        input$team_performance_input,
        input$team_performance_year_input[1],
        input$team_performance_year_input[2]
      )
    })
  reactive_team_bowling_performance <-
    reactive({
      get_team_bowling_performance(
        input$team_performance_input,
        input$team_performance_year_input[1],
  ...
  output$team_batting_performance <-
    renderDataTable({
      reactive_team_batting_performance()
    },
    selection = 'none',
  ...
  output$team_bowling_performance <-
    renderDataTable({
      reactive_team_bowling_performance()
    },
    selection = 'none',
